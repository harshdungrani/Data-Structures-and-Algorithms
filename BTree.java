...
public class BTree{
	...
	private static Node splitNode(Node node, int key){
		System.out.println("Splitting of Node");
		int mid = node.size/2;
	...
	public static void add(int data){
		...
		Node temp = root;
		while(!temp.leaf || (temp.size==t-1 && temp.children[0]==null)){
			if(temp.size==t-1){
	...
	public static void display(Node node, int level){
		...
		System.out.print("Level "+ level +"	"+"Data in node:");
	...
	public static void main(String [] args){
		...
		System.out.println("Enter the number of values to be added in BTree of order 3: ");
		int n=sc.nextInt();
		for(int i=0;i<n;i++){
			System.out.println("\nEnter the value: ");
			int val=sc.nextInt();
